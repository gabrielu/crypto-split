<script setup>
const props = defineProps(['currency', 'error', 'inputId'])
const emit = defineEmits(['setAllocation'])

import {onMounted, ref, watchEffect} from 'vue'
import { initDropdowns } from 'flowbite'

const investable = ref('1000');

const investableChanged = (event, investableVal = +event.target.value) => {
  investable.value = investableVal
  emitChange(investableVal)
}

const emitChange = (val) => {
  emit('setAllocation', val)
}
...
onMounted(() => {
  ...
  const currencyInput = document.getElementById(props.inputId)

  const updateCurrencyInput = () => {
    currencyInput.value = rangeInput.value
    emitChange(rangeInput.value)
  }
...
watchEffect(async () => {
  emit('setAllocation', investable.value)
})
</script>

<template>
<div class="max-w-[24rem] mx-auto pb-8">
  <div class="flex mb-4">
  ...
  <div v-if="props.error" class="flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
    ...
    <div>
      <span class="font-medium">Alert!</span> {{ props.error }}
    </div>
  </div>
